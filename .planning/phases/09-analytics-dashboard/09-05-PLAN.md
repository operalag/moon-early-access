...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app/admin/dashboard/page.tsx"
      to: "API routes with date params"
...
2. Update API calls to accept date filtering:
   - Add ?from=&to= query params to user, engagement, points, campaigns APIs
   - APIs should filter by created_at within range
   - Update each API route to parse from/to query params and add WHERE clause
...
5. Define CSV headers for each export:
  ...
  <verify>
    ...
    - `curl 'http://localhost:3000/api/admin/analytics/users?from=2026-01-01&to=2026-01-31'` returns filtered data
    - `curl 'http://localhost:3000/api/admin/analytics/engagement?from=2026-01-01&to=2026-01-31'` returns filtered data
  </verify>
  <done>
    ...
    - All 4 API routes (users, engagement, points, campaigns) accept and filter by date params
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    9. Verify Leaderboard Trends section shows position changes
  </how-to-verify>
...
8. API routes correctly filter by date range when params provided
</verification>

<success_criteria>
- Export buttons on all data sections
- Date range filtering affects user growth, engagement, points, campaigns charts
...
- Leaderboard dynamics section shows top movers
</success_criteria>
